...
class AwsWafAdapter:
    ...
    def _click_ui_solution(self, solution: list[int], image_count: int) -> None:
        ...
        if len(grid_buttons) == image_count:
            ...
            for index in solution:
                ...
                canvas.click(
                    position={
                        "x": column * (cell_width + gap) + cell_width / 2,
    ...
    def _wait_for_ui_voucher(self, previous_voucher_count: int) -> int:
        ...
        logging.error(
            "AWS WAF Confirm did not complete voucher=%d verify_statuses=%s voucher_statuses=%s token_length=%d ui=%s",
            len(self.state.voucher_urls) - previous_voucher_count,
            self.state.verify_statuses,
            self.state.voucher_statuses,
            self._browser_waf_token_length(),
            self._captcha_ui_text(),
        )
        raise AwsWafError("AWS WAF Confirm did not produce a browser voucher")
    ...
    def solve(self, problem_url: str) -> dict[str, int | bool]:
        ...
        logging.info(
            "AWS WAF UI before Confirm text=%s selected=%d",
            self._captcha_ui_text(),
            len(solution),
        )
        self._click_ui_solution(solution, len(images))
        logging.info("AWS WAF Confirm clicked")
        token_length = self._wait_for_ui_voucher(previous_voucher_count)
        return {
            "success": True,
            "images": len(images),
