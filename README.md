...
面板可保存带手工时间的账号记录，账号与密码使用当前面板密码派生的密钥加密保存在 `data/secure_store.json`；列表只返回邮箱与时间。成功拿到完整四组 VOXI 码的账号会自动写入同一密文中的优惠码库，按账号和下次日期归档，四个码和日期可分别复制。在线修改面板密码时会重新加密账号记录和优惠码库，`.env` 仅用于首次创建存储。实时日志通过同源流式响应展示，登录页面、Turnstile、按钮启用和提交结果均使用适合慢速网站的延长等待时间；每条日志带任务相对耗时，backend Docker 日志也会保留同一份记录。

Turnstile、登录和代码采集必须由同一次 FlareSolverr Camoufox 会话完成：backend 将账号传给内网 solver，Camoufox 在同一页面处理 Cookie、Turnstile、账号填写、登录提交和 VOXI 页面 GraphQL POST，代理也只由这一浏览器使用，不再把 token 或 Camoufox 的 Firefox UA 注入第二个 Chromium。进入 VOXI 页面后先等待 10 秒让 viewer token 和页面脚本稳定；当前每个计划的 POST 只尝试 1 次，页面没有内容或 viewer token 时也只检查 1 次，不重复消耗账号。代码采集不点击 `affiliateLink`，只返回四个计划的码和结束日期。Cookie 同意弹窗会等待延迟挂载，并使用原生浏览器点击后确认遮罩隐藏；挑战 iframe 尚未挂载时，solver 会从 Turnstile 容器坐标执行真实鼠标点击，并在日志中标注 `widget-mouse` 或 `iframe-mouse` 策略。登录端使用带 `client_id` 和 `user_return_to` 的 Student Beans OAuth 登录入口，solver 会在同一上下文中等待 `Log in` 按钮启用、提交并返回 API 状态、最终 URL 和登录截图。慢速或代理网络下表单会保持校验，异常日志会注明加载、填写、求解、提交和代码采集阶段。FlareSolverr 不可用、未返回 token 或同页登录未确认成功时，本次代理尝试立即失败；当前每个账号只执行 1 次完整尝试，失败后直接返回，便于定位固定流程问题。只有四个计划全部成功才会归档。日志只显示验证状态、来源、长度、API 状态、代码计划结果和重试信息，不显示 token、码、日期、Cookie、账号密码或代理凭据。多个账号任务并发运行，代理池用独占租约防止同一时刻重复使用同一地址，代理不足时排队；没有代理时直连任务串行。登录任务运行时可以点击“结束任务”手动停止。

登录判断先等待当前地址离开登录路径且密码输入框消失，然后等待同一标签页出现 `Account Settings` 或已经落到 `www.studentbeans.com/uk`。如果仍在 accounts 域，随后在同一 Camoufox 标签页以原始登录页为 Referer 访问 `/uk/authorisation/passthrough`，完整跟随 HAR 中的 OAuth authorize → callback → `www.studentbeans.com/uk` 链路，确认站点域回调完成后才打开 VOXI 目标页面；任一阶段未到预期时本次尝试失败。失败或调试模式最多保留 3 张最新阶段截图，并在日志中记录每步状态、响应码、脱敏 URL、标题、DOM 状态、控件状态、Cookie 名称和存储键名。任务运行时点击“结束任务”会同时请求 backend 取消活动任务并中断 SSE，避免只停前端显示而留下后台任务。面板代理池会持久保存在当前浏览器的本地存储中。
